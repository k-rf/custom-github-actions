...
var lib_core = __nccwpck_require__(139);
...
const run = () => {
    try {
        lib_core.info("------ meta ------");
        lib_core.info(Object.entries(action.meta.inputs)
            .map(([key, value]) => `${key}: { default: ${value.default}, required: ${value.required} }`)
            .join("\n"));
        lib_core.info("------ inputs ------");
        lib_core.info(Object.entries(action.inputs)
            .map(([key, value]) => `${key}: ${value}`)
            .join("\n"));
    }
    catch (err) {
